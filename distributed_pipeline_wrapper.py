...
def upload_outputs(date, yaml_dictionary):
    ...
    if args.hpc: 
        ...
        sp.call(f"ssh -vvv filexfer 'cd {cwd}' '&& imkdir -p {irods_output_path}' '&& icd {irods_output_path}' '&& iput -rfkV {date} > upload_log.txt' '&& exit'", shell=True) #-X upload_log --retries 2 
